...
const {identity, obj_entries_map, xs_group} = require('@leonardpauli/utils/src/misc.js')
...
async function nordvpn_clean () {
	...
	// inspect country
	if (true) {
		const country_code_get = await country_code_get_get()
		const info = {
			country: histogram_get(raw, {key_get: row=> country_code_get(row.country)})
		}
...
const country_code_get_get = async ()=> {
...
const histogram_get = (xs, {key_get = identity, limit = 0} = {})=> {
	const groups = xs_group(xs, {key_get})
	const histogram = Object.fromEntries([...groups]
		.map(([k, v])=> [k, v.length]).filter(([k, c])=> c>limit))
	return histogram
}
